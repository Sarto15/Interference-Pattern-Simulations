...
def plot_interactive_fringes(
    ...
    z = 0.005,               # Axial distance from point source (m)
    thetaDeg = 0.0           # Angle propagation of the two waves
):
    ...
    # Phase difference for spherical fringes (exact form)
    r = np.sqrt((X - z * np.tan(theta))**2 + Y**2 + z**2)  # Tilt in x-direction for simplicity
    deltaPhi = k * (r - z * np.cos(theta))
    ...
    plt.imshow(Intensity, cmap="gray", extent=[x.min() * 1e6, x.max() * 1e6, y.min() * 1e6, y.max() * 1e6])
    plt.title(f"Interactive Spherical Fringe Pattern\n(Coherence={Coherence:.2f}, I1={Intensity1:.2f}, I2={Intensity2:.2f}, z={z:.4f} m, θ={thetaDeg:.1f}°, Vis={Visibility:.1f})")
    plt.xlabel("x (µm)")
    plt.ylabel("y (µm)")
...
interact(
    ...
    z=FloatSlider(min=0.001, max=0.05, step=0.001, value=0.005, description="z (m)"),
    thetaDeg=FloatSlider(min=-20.0, max=20.0, step=0.5, value=0.0, description="θ (deg)")
)
